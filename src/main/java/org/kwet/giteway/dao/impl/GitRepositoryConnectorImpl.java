...
public class GitRepositoryConnectorImpl extends AbstractGitConnector implements GitRepositoryConnector {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.kwet.giteway.dao.GitRepositoryConnector#find(java.lang.String,
	 * java.lang.String)
	 */
	@Override
	public Repository find(String owner, String name) {
		checkRepository(owner, name);
		GitHubRepository gitHubRepository = gitHttpClient.executeRequest(GET_REPOSITORY, GitHubRepository.class, owner, name);
	 ...
	 * 
	 * @see
	 * org.kwet.giteway.dao.GitRepositoryConnector#findCollaborators(java.lang
	 * .String, java.lang.String)
	 */
	@Override
	public List<User> findCollaborators(String owner, String name) {
		checkRepository(owner, name);
		GitHubUser[] gitHubUsers = gitHttpClient.executeRequest(GET_COLLABORATORS, GitHubUser[].class, owner, name);
	 ...
	 * 
	 * @see
	 * org.kwet.giteway.dao.GitRepositoryConnector#findCommits(java.lang.String,
	 * java.lang.String)
	 */
	@Override
	public List<Commit> findCommits(String owner, String name) {
		checkRepository(owner, name);
		GitHubCommit[] commitWrappers = gitHttpClient.executeRequest(GET_COMMITS, GitHubCommit[].class, owner, name);
		List<GitHubCommit> filteredWrappers = filter(having(on(GitHubCommit.class).getCommit(), notNullValue()), commitWrappers);
		filteredWrappers = filter(having(on(GitHubCommit.class).getCommitter(), notNullValue()), filteredWrappers);
